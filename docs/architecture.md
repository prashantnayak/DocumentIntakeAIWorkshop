...
resource group `rg-intakeai-dev-swc`. Intake is a polling Blob trigger; there is
...
| 2 | The Python Blob extension polls through the private endpoint with the Function user-assigned identity. `PhiStorage` defines only `blobServiceUri`, `credential`, and `clientId`; no PHI-storage queue endpoint is configured. Checkpoints and poison notifications use private `AzureWebJobsStorage`. | `src/functions/function_app.py` `DocumentBlobStarter`; `infra/modules/function-app.bicep` |
| 3 | Registration resolves and records the exact blob version ID and ETag in `dbo.ProcessingInbox`. Duplicate trigger deliveries return the existing `DocumentId`. | `src/functions/intake/registration_service.py`; `sql/migrations/003_processing_state.sql` |
...
| 8 | `business-rules-workflow` reads the sidecar, verifies the document identity, and authoritatively applies confidence, high-risk-type, and required-field rules. | `src/logicapps/workflows/business-rules-workflow/workflow.json` |
...
Repeated Blob-trigger failures are placed in runtime storage queue
`webjobs-blobtrigger-poison`. This is operational trigger state, not an
application transport. Runtime storage already has a Queue private endpoint,
and the Function identity has Storage Queue Data Contributor there.
...
| Identity | Use |
|---|---|
| Function user-assigned identity | Blob source and runtime bindings, exact blob access, Document Intelligence, SQL stored procedures, Key Vault secret resolution, deployment-package pull, and user-delegation SAS creation. |
...
- SQL-backed idempotency and leases tolerate at-least-once trigger delivery and
  workflow retries.
- Durable timers avoid holding an execution thread during status polling or SLA
  waits.
- Poison-queue, stale-inbox, Function/Durable failure, workflow failure,
